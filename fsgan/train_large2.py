...
def main(exp_dir, train_dir, val_dir=None, workers=0, iterations=['60K'], epochs=[20], start_epoch=None,
         lr_gen=(1e-4,1e-4), lr_dis=(1e-4,1e-4), batch_size=[6, 2], resolutions=(128, 256), resume_dir=None, seed=None,
    ...
    train_dataset = obj_factory(train_dataset, st=0, end=218218, root=train_dir, img_list='/home/dafc/HRNet-Facial-Landmark-Detection/outputcuowu/WFLW/face_alignment_wflw_hrnet_w18/filepath2.txt',
                                landmarks_list='/home/dafc/HRNet-Facial-Landmark-Detection/outputcuowu/WFLW/face_alignment_wflw_hrnet_w18/predictions2.npy',
                                bboxes_list='/home/dafc/HRNet-Facial-Landmark-Detection/outputcuowu/WFLW/face_alignment_wflw_hrnet_w18/predictions2.npy',
                                pair_transform=None,
                                transform1=img_transforms1, transform2=img_transforms2)
    if val_dir:
        val_dataset = obj_factory(val_dataset, st=218218,end=300000,root=val_dir, img_list='/home/dafc/HRNet-Facial-Landmark-Detection/outputcuowu/WFLW/face_alignment_wflw_hrnet_w18/filepath2.txt',
                                landmarks_list='/home/dafc/HRNet-Facial-Landmark-Detection/outputcuowu/WFLW/face_alignment_wflw_hrnet_w18/predictions2.npy',
                                bboxes_list='/home/dafc/HRNet-Facial-Landmark-Detection/outputcuowu/WFLW/face_alignment_wflw_hrnet_w18/predictions2.npy',
                                pair_transform=None,
                                transform1=img_transforms1, transform2=img_transforms2)
    ...
    for ri in range(start_res_ind, len(resolutions)):
        ...
        if val_dir:
            if res_iterations is None:
                ...
                val_iterations = (res_iterations * len(val_dataset.classes)) // (len(train_dataset.classes)*80)
        ...
        for epoch in range(start_epoch, res_epochs):
            ...
            for i, (img1, landmarks1, img_seg1, target1, img2, landmarks2, img_seg2, target2,imgref,landmarksref) in enumerate(pbar):
                ...
                if i % (log_freq*100) == 0:
                    ...
                    grid = img_utils.make_grid(img1[0], img_pred, img2[0])
                    #grid = img_utils.make_grid(img1[0],img_seg1[0].cuda(),imgref[0][1], imgref[0][2], torch.cat((landmarks1[0],landmarks1[0],landmarks1[0]),dim=1))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_dir',type=str,default='exp_dir_large2',
                        help='path to experiment directory')
    parser.add_argument('-t', '--train', type=str, metavar='DIR',default='exp_dir_large2',
